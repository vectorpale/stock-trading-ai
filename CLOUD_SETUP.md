...
**自动运行**（盘前 2 小时生成信号，方便开盘前阅读）：
- **港股信号**：每个交易日北京时间 07:30 自动执行（港股 09:30 开盘前 2 小时）
- **美股信号**：每个交易日北京时间 20:30 自动执行（美股开盘前约 1-2 小时）
...
| 文件 | 功能 | 触发方式 |
|---|---|---|
| `.github/workflows/daily_signals.yml` | 每日信号生成 | 定时（港股盘前 07:30 + 美股盘前 20:30 北京时间）+ 手动 |
